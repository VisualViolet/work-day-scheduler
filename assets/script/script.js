...
$(function () {
  ...
  // compares current time to the element time and applies or removes classes accordingly
  function checkTime() {
    var currentTime = parseInt(dayjs().format("H"));
    $(".time-block").each(function () {
      var elTime = parseInt($(this).attr("id").split("hour-").pop());
      if (elTime < currentTime) {
        $(this).removeClass("present");
        $(this).removeClass("future");
        $(this).addClass("past");
      } else if (elTime > currentTime) {
